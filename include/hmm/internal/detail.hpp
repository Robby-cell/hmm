...
template <class Ty> struct type_identity {
    using type = Ty;
};
...
template <class Ty> struct remove_cvref : type_identity<remove_cvref_t<Ty>> {};
...
HMM_CONSTEXPR_14 inline size_t
IndexWithoutProbing(const std::size_t h1, const std::size_t capacity) noexcept {
...
template <class T> struct IsDirectInitializable {
    template <class U, class... Args>
    static auto is_it(Args&&... args)
        -> AlwaysTrue<decltype(U(std::forward<Args>(args)...))>;
    template <class U, class... Args> static auto is_it(...) -> std::false_type;
};

template <class DirectInitializable> struct Constructor;

template <> struct Constructor<std::true_type> {
    template <class T, class... Args> static T construct(Args&&... args) {
        return T(std::forward<Args>(args)...);
    }
};

template <> struct Constructor<std::false_type> {
    template <class T, class... Args> static T construct(Args&&... args) {
        return T{std::forward<Args>(args)...};
    }
};
...
template <class T, class U> T exchange(T& self, U&& other) {
    T old = std::move(self);
    self = std::forward<U>(other);
    return old;
}

template <class T, class U> struct ForwardLike {
    using type = U&&;
};

template <class T, class U> struct ForwardLike<T&&, U> {
    using type = U&&;
};
...
template <class T, class U> struct ForwardLike<const T&&, U> {
    using type = const U&&;
};
...
template <class... Ts> struct TypePack;

template <std::size_t I, class T, class... Ts> struct TypeAtIndexImpl {
    using type = typename TypeAtIndexImpl<I - 1, Ts...>::type;
};

template <class T, class... Ts> struct TypeAtIndexImpl<0, T, Ts...> {
    using type = T;
};

template <std::size_t I, class TP> struct TypeAtIndex;

template <std::size_t I, class... Ts> struct TypeAtIndex<I, TypePack<Ts...>> {
...
template <std::size_t Index, class Default, class... Args>
...
template <std::size_t Index, class Default, class First, class... Rest>
...
template <std::size_t Index, class Default>
